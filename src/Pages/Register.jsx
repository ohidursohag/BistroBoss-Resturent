...
const Register = () => {
   ...
   const { registerWithEmailPass, googleLogin, logOut } = useAuth()
...
const handleRegister = async (e) => {
   ...
   try {
      // User Registration
      const result = await registerWithEmailPass(email, password)
      // Save username & profile photo
      await updateProfile(result.user, { displayName: name, photoURL: photoUrl })
      
      
      // get Access-token
      // const getAccessToken = await axiosSecure.post('/auth/access-token', { email: result.user.email });
      const getAccessToken = await accessToken(result?.user?.email)
      console.log(getAccessToken);
      if (getAccessToken?.success) {
         // save/update user info to database
         const data = await saveUserData(result?.user)
         console.log(data);
         ...
         <div className={'flex w-[90%] mx-auto gap-5 justify-between items-center'}>
            ...
            <div className='w-2/5 my-10'>
               ...
               <div className='flex items-center justify-center gap-5'>
                  ...
                  <div onClick={handleRegisterWithGoogle} className='cursor-pointer'>
                     <svg width="52" height="52" viewBox="0 0 52 52" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <circle cx="26" cy="26" r="25" fill="#F1F2F4" stroke="#444444" strokeWidth="2" />
